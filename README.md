...
where $\mathbf{c}_i$ is the FSQ coordinate of code $i$ on the integer lattice. We compare three kernels (Gaussian $e^{-d^2/2\sigma^2}$, Laplace $e^{-d/\sigma}$, Cauchy $1/(1+d^2/\sigma^2)$) and settle on a narrow Gaussian ($\sigma = 0.7$): its squared-exponential tails decay faster than Laplace or Cauchy, so distant neighbours receive negligible mass ($k(2) \approx 0.02$), while $\sigma = 0.7$ still places meaningful mass on the immediate neighbour ($k(1) \approx 0.37$, matching what Laplace gives at $\sigma = 1$). Under joint training the codebook drifts during the run, so per-dimension sensitivity weights calibrated from a snapshot would go stale; we use an isotropic kernel with bandwidth set from a first-neighbour rule rather than fit to data. This formulation generalizes to any discrete latent space with a coordinate metric.
