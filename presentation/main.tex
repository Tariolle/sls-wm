...
\begin{frame}{Data Collection}
  ...
  \begin{itemize}
    ...
    \item $\sim$3,600 episodes, $\sim$180K frames $\approx$ \textbf{1h40 of gameplay}
  \end{itemize}
\end{frame}
...
\begin{frame}{Transformer: Training Results}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \textbf{Best model} (200 epochs, LR $10^{-3} \to 10^{-4}$):
    \begin{itemize}
      \item Val accuracy: \textbf{32.85\%}
      \item Death accuracy: 97.9\%
      \item Overfitting gap: $\sim$3.5pp
    \end{itemize}

    \vspace{0.5em}
    \textbf{Key insight}: shift augmentation eliminated overfitting, enabling $10\times$ higher LR

    % TODO: update with structured label smoothing results
...
\begin{frame}{Controller: Architecture}
  \[
    \text{action} = \begin{cases} \text{jump} & \text{if } \sigma(\mathbf{W} \cdot \mathbf{h}_t + b) > 0.5 \\ \text{idle} & \text{otherwise} \end{cases}
  \]

  \textbf{257 parameters} ($\mathbf{W} \in \mathbb{R}^{256}, b \in \mathbb{R}$). Trained via \textbf{CMA-ES}.

  \vspace{0.5em}
  \textbf{Why CMA-ES?}
  \begin{itemize}
    \item 257 params: too few for SGD to be efficient
    \item No differentiable reward (binary survival)
    \item Derivative-free, handles non-smooth landscapes
  \end{itemize}
\end{frame}

\begin{frame}{Controller: Training Protocol}
  \begin{itemize}
    \item Near-death context sampling: start 6--12 frames before death
    \item Fitness $=$ frames survived in dream before death prediction
    \item DreamerV3-style percentile normalization of fitness across episodes
  \end{itemize}

  % TODO: controller results
  \textit{[Controller results: TODO]}
\end{frame}
...
\begin{frame}{What Didn't Work (or Didn't Help)}
  \begin{itemize}
    ...
    \item \textbf{First CMA-ES run}: $\sigma$ diverged to 19. Most sampled contexts were safe, zero fitness signal
...
\begin{frame}{Limitations \& Future Work}
  \begin{itemize}
    ...
    \item \textbf{Death prediction timing}: low probability during safe sections. Controller training requires near-death context sampling
